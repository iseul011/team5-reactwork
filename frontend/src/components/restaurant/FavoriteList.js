...
function FavoriteList() {
  ...
  const displayMarkersOnMap = (restaurants) => {
    ...
    restaurants.forEach((restaurant) => {
      ...
      // InfoWindow 클릭 시 상세보기를 보여줄 수 있는 클릭 이벤트 추가
      ...
      window.naver.maps.Event.addListener(marker, "click", () => {
        ...
        // InfoWindow가 생성된 후 클릭 이벤트를 추가
        setTimeout(() => {
          const infoWindowElement = document.getElementById(`infoWindow-${restaurant.restaurant.rnum}`);
          if (infoWindowElement) {
            infoWindowElement.addEventListener("click", () => {
              handleRestaurantClick(restaurant.restaurant); // InfoWindow 클릭 시 상세보기로 이동
            });
          }
        }, 0); // 0ms 딜레이로 InfoWindow 생성 후 이벤트 리스너 추가
      });
